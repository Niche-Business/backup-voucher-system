"""
Wallet System Blueprint for Schools/Care Organizations
Flask blueprint containing all wallet-related endpoints
"""
...
# from main import db, User, Voucher, WalletTransaction

wallet_bp = Blueprint('wallet', __name__)


def init_wallet_blueprint(db_instance, user_model, voucher_model, wallet_transaction_model):
    """Initialize the blueprint with database models"""
    global db, User, Voucher, WalletTransaction
    db = db_instance
    User = user_model
    Voucher = voucher_model
    WalletTransaction = wallet_transaction_model
...
@wallet_bp.route('/api/school/wallet/balance', methods=['GET'])
def get_school_wallet_balance():
    ...
    try:
        ...
        return jsonify({
            ...
            'allocated_balance': float(user.allocated_balance) if hasattr(user, 'allocated_balance') else 0,
...
@wallet_bp.route('/api/school/wallet/transactions', methods=['GET'])
...
@wallet_bp.route('/api/school/wallet/add-funds', methods=['POST'])
...
@wallet_bp.route('/api/school/vouchers/issue', methods=['POST'])
def school_issue_voucher():
    ...
    try:
        ...
        voucher = Voucher(
            ...
            status='active',
            assign_shop_method=assign_shop_method if hasattr(Voucher, 'assign_shop_method') else None,
            deducted_from_wallet=True,
            wallet_transaction_id=wallet_transaction.id
        )
        
        if hasattr(voucher, 'recipient_selected_shop_id') and assign_shop_method == 'specific_shop' and shop_id:
...
@wallet_bp.route('/api/school/vouchers', methods=['GET'])
    ...
    try:
        ...
        for v in vouchers:
            vouchers_data.append({
                ...
                'assign_shop_method': v.assign_shop_method if hasattr(v, 'assign_shop_method') else None
            })
...
@wallet_bp.route('/api/school/reports/funds-summary', methods=['GET'])
    ...
    try:
        ...
        for t in transactions:
            ...
            if t.transaction_type in ['credit', 'allocation']:
                daily_summary[date_key]['credits'] += float(t.amount)
            elif t.transaction_type == 'debit':
                daily_summary[date_key]['debits'] += float(t.amount)
...
@wallet_bp.route('/api/school/reports/voucher-spend', methods=['GET'])
...
@wallet_bp.route('/api/admin/school/<int:school_id>/allocate-funds', methods=['POST'])
def admin_allocate_funds_to_school(school_id):
    ...
    try:
        ...
        if hasattr(school_user, 'allocated_balance'):
