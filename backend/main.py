...
def school_issue_voucher():
    ...
    try:
        ...
        # Check if school has sufficient wallet balance (use allocated_balance for schools)
        total_balance = (user.balance or 0) + (user.allocated_balance or 0)
        if total_balance < amount:
            return jsonify({'error': f'Insufficient wallet balance. Current balance: £{total_balance:.2f}, Required: £{amount:.2f}'}), 400
        ...
        # Prioritize deducting from allocated_balance first, then from balance
        balance_before = user.balance + (user.allocated_balance or 0)
        if user.allocated_balance and user.allocated_balance >= amount:
            user.allocated_balance -= amount
        elif user.allocated_balance and user.allocated_balance > 0:
            remaining = amount - user.allocated_balance
        ...
        return jsonify({
            ...
            'remaining_balance': float((user.balance or 0) + (user.allocated_balance or 0))
        }), 201
